...
for slicetoremove in range(0, shape[2]):
            ...
            for j in range (2, shape[1]-2):
                if k != slicetoremove:
                    ...
                    X_train.append([data[i][j][k], data[i+1][j][k], data[i][j+1][k], 
                          ...
                          data[i+2][j-2][k], data[i+2][j-1][k], data[i+2][j][k], 
                          data[i+2][j+1][k], data[i+2][j+2][k], data[i+1][j-2][k], 
                          data[i+1][j+2][k], data[i][j-2][k], data[i][j+2][k], 
                          data[i-1][j-2][k], data[i-1][j+2][k], data[i-2][j-2][k], 
                          data[i-2][j-1][k], data[i-2][j][k], data[i-2][j+1][k],
                          data[i-2][j+2][k]])
    ...
    predictedarray = np.reshape(predicted, (shape[0]-4, shape[1]-4))
    actualarray = np.reshape(ms_slice, (shape[0]-4, shape[1]-4))
